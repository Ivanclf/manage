...
### 二维码 GET /activity/{id}/qrcode

- 参数: `width`、`height`（可选，默认值为300px）
- 响应：目前为png图，后面实现OSS存储功能后为`Result<String>`，成功时`data`字段为获得图片的URL。（二维码用于指向活动宣传网页）
...
- 请求体（JSON）：
    ...
    ```
- 说明：同一活动每个手机号只能报名一次
- 响应：`Result<Long>`，成功时 `data` 为报名记录 id
...
- 查询参数：
  - `width`（可选，默认 300）
  - `height`（可选，默认 300）
- 响应：目前为png图，后面实现OSS存储功能后为`Result<String>`，成功时`data`字段为获得图片的URL。（二维码内容指向报名页）
...
- 查询参数：
  - `width`（可选，默认 300）
  - `height`（可选，默认 300）
- 响应：目前为png图，后面实现OSS存储功能后为`Result<String>`，成功时`data`字段为获得图片的URL。（二维码内容用于签到）
